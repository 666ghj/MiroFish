      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- Show a hint if there are more files -->
        ...
        <!-- Card title (first 20 chars of the simulation requirement) -->
...
const selectedProject = ref(null)  // The project currently selected (for the modal)
let observer = null
...
// Card layout config - tuned for a wider aspect ratio
const CARDS_PER_ROW = 4
const CARD_WIDTH = 280  
const CARD_HEIGHT = 280 
const CARD_GAP = 24

// Compute the container height style dynamically
const containerStyle = computed(() => {
  ...
  // Expanded: compute height from the number of cards
  ...
  // Required height: rows * card height + (rows - 1) * gap + small bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Compute the cards on the current row to keep each row centered
    ...
    // Expand downward; add some space below the title
    ...
    const x = offset * 35
    // Tune starting position: close to the title but with some spacing
...
// Pick a CSS class based on round progress
...
// Build a title from the simulation requirement (first 20 chars)
...
// Format the simulation_id for display (first 6 chars)
...
// Get the label text for the file-type tag
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the pending target state (record the latest target whether or not we're animating)
        pendingState = shouldExpand

        // Clear any prior debounce timer (a new scroll intent overrides the old one)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }

        // If already animating, just record the state and handle it after the animation finishes
        ...
        // Debounce the state toggle to avoid rapid flicker.
        // Use a short delay (50ms) when expanding, longer (200ms) when collapsing for stability
        const delay = shouldExpand ? 50 : 200

        expandDebounceTimer = setTimeout(() => {
          // Check whether an animation is still running
          if (isAnimating) return

          // Check whether the pending state is still valid (it may have been overridden by a subsequent scroll)
          ...
          // After the animation finishes, release the lock and check for any queued state change
          setTimeout(() => {
            isAnimating = false

            // After animation, check for a new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Wait a brief moment so we don't toggle too fast
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    {
      // Use multiple thresholds to smooth detection
      threshold: [0.4, 0.6, 0.8],
      // Tighten rootMargin: shrink the viewport bottom upward so the user must scroll further before expansion triggers
...
// Watch route changes; reload data when returning to the home page
...
onMounted(async () => {
  // Wait for the DOM to render before loading data
  await nextTick()
  await loadHistory()

  // Initialize the observer once the DOM has rendered
  setTimeout(() => {
    initObserver()
  }, 100)
})

// When used with keep-alive, reload data on activation
onActivated(() => {
  loadHistory()
})
...
/* Use a CSS background pattern for a fixed-spacing square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchor at top-left so height changes only extend the bottom and don't shift the existing grid */
...
.cards-container {
  ...
  /* min-height is computed dynamically in JS based on card count */
}
