    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - shown when generation completes -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content done, but the full section may not be complete yet) -->
                  ...
                  <!-- Section Complete (entire section finished) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After the DOM updates, adjust scroll so the button stays in the same place
...
const parseInsightForge = (text) => {
  ...
  try {
    // Extract the analysis question (regex matches the English-formatted backend output)
    ...
    // Extract statistics - matches the "Relevant predicted facts: N" format
    ...
    // Extract sub-questions - take all of them, no cap
    ...
    // Extract key facts - take all of them, no cap
    ...
    // Extract core entities - take all, including summary and related-fact counts
    ...
    // Extract relation chains - take all, no cap
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract involved entities - take all, no cap
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const reasonMatch = text.match(/### Reasoning for Selecting Interviewees\n([\s\S]*?)(?=\n---\n|\n### Interview Transcripts)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Parse the per-person selection rationale
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: 1. **Alumni_345 (index=1)**: As an alumnus of Wuhan University...
        ...
        // Format 2: - Select(ed) name (index X): reason
        // e.g.: - Selected Parent_601 (index 0): As a representative of the parent group...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:Selected|Select|Choose|Chose)\s+([^(]+?)(?:\s*\(index\s*=?\s*\d+\))?\s*[:：]\s*(.*)/i)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*\*\*([^*(]+?)(?:\s*\(index\s*=?\s*\d+\))?\*\*\s*[:：]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        } else if (currentName && line.trim() && !line.match(/^Not selected|^In summary|^Final selection|^Overall/i)) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract the title (e.g. "Student", "Educator", etc.)
      ...
      if (qMatch) {
        ...
        // Split questions on numeric markers
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // If the first question is prefixed by "1.", handle it specially
      ...
      // Extract responses - separated by Twitter and Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*Key Quotes|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Separate the Twitter and Reddit responses
        ...
        // Platform fallback logic (compatible with the old format that only had one platform marker)
        if (!twitterMatch && redditMatch) {
          // Only a Reddit reply — copy it as the default display only if it isn't placeholder text
          ...
          // No platform markers at all (very old format) — treat the whole text as the answer
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes (handles several quote-character variants)
      const quotesMatch = block.match(/\*\*Key Quotes:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // First try the > "text" format
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Found\s*(\d+)\s*relevant items/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // Extract related facts - take all, no cap
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Per question-and-answer pair, track an independent platform selection
    ...
    // Try splitting the answer by question numbers
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" (English format, the new backend format)
      ...
      // If nothing matched, fall back to the "number." format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // If no numbering or only a single match, return the whole text
      if (matches.length <= 1) {
      ...
      // Extract each part by its numbering
      const parts = []
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: return the whole answer for the first question and empty for the rest
      return qIdx === 0 ? answer : ''
    }
    
    // Check whether a question has answers from both platforms (filter out placeholder text)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real (non-placeholder) answers and the content differs
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Q&A Conversation Thread - one-question-per-answer style
...
// Currently active step (for the top display)
const activeStep = computed(() => {
  ...
  // If none is active, return the last completed step
...
const renderMarkdown = (content) => {
  ...
  // Strip the leading h2 (## xxx) since the section title is already shown above
  ...
  // Handle newlines - blank lines become paragraph breaks, single newlines become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean up <p><br> followed by a block-level element (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Collapse consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Remove a <br> right before a paragraph that follows a block element
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-contiguous ordered-list numbering: when single-item <ol>s are separated by paragraph content, keep the count incrementing
...
const getLogLevelClass = (log) => {
  // Match log levels emitted by the backend (case-insensitive English)
  if (/error/i.test(log)) return 'error'
  if (/warning/i.test(log)) return 'warning'
  // INFO uses the default color and is not styled as success
  return ''
}
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Make sure the loading state is cleared
            emit('update-status', 'completed')
            stopPolling()
            // Scroll handling is done uniformly in the nextTick after the loop
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If the task is complete, scroll to the top; otherwise stay at the bottom to follow the latest logs
            if (isComplete.value) {
...
// Extract the final answer - pull the section content out of an LLM response
const extractFinalContent = (response) => {
  if (!response) return null

  // Try to extract the content inside <final_answer> tags
  ...
  // Try to find content after "Final Answer:" (handles multiple formats)
  ...
  // If it starts with ##, #, or >, it may already be plain markdown
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }

  // If the content is long and contains markdown formatting, try stripping the "Thought:" prefix
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove the "Thought:" prelude
...
/* Console Logs - kept in sync with Step3Simulation.vue */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
