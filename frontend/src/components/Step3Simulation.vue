          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- Generic fallback: unknown action type, or has content but wasn't handled above -->
...
const props = defineProps({
  ...
  minutesPerRound: {
    type: Number,
    default: 30 // Default of 30 minutes per round
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
// Computed
// Show actions in chronological order (most recent at the bottom)
...
// Twitter platform's simulated elapsed time
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Reddit platform's simulated elapsed time
...
const resetAllState = () => {
  ...
  stopPolling()  // Stop any pre-existing polling
}
...
const doStartSimulation = async () => {
  ...
  // Reset all state first so this run isn't affected by the previous one
  resetAllState()
...
// Track the previous round per platform so we can detect changes and log them
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Additional check: the backend may not have updated runner_status yet, but the platforms have reported completion.
      // Detect this via the twitter_completed and reddit_completed flags.
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform finished, check whether every enabled platform did.
  // A platform is considered enabled if actions_count > 0 or running was ever true.
  ...
  // Verify every enabled platform has completed
