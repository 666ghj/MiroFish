...
const selectReportAgentChat = () => {
  ...
  // Restore the Report Agent's chat history
...
const sendMessage = async () => {
  ...
  try {
    ...
    scrollToBottom()
    // Auto-save the chat history to the cache
    saveChatHistory()
  }
}
...
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    // Correct data path: res.data.result.results is a dictionary
    // Shape: {"twitter_0": {...}, "reddit_0": {...}} or single-platform {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData

    // Convert the dictionary to an array, preferring the reddit platform reply
    let responseContent = null
    const agentId = selectedAgentIndex.value

    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Prefer the reddit reply, fall back to twitter
      ...
      // Backwards compatibility with array shape
...
/* Fix ordered-list numbering - use CSS counters so multiple <ol>s number continuously */
.message-text {
