...
const showEdgeLabels = ref(true) // Edge labels visible by default
const expandedSelfLoops = ref(new Set()) // Expanded self-loop items
const showSimulationFinishedHint = ref(false) // Post-simulation hint visible
const wasSimulating = ref(false) // Track whether the previous state was simulating
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to not-simulating — show the post-simulation hint
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Toggle a self-loop item's expanded/collapsed state
...
const renderGraph = () => {
  ...
  // Process edge data: count edges between each pair of nodes and assign per-pair indexes
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // Self-loop - collect into the array
  ...
  // Track which edge index we're up to within each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop edge - emit only one merged self-loop per node
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Store the details of every self-loop edge
        }
      })
      return
    }
    ...
    // Whether this edge's direction matches the normalized direction (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid

    // Compute curvature: spread multiple edges apart; single edge is straight
    let curvature = 0
    if (totalCount > 1) {
      // Spread curvature evenly so each edge is visibly distinct.
      // The curvature range grows with edge count — more edges means a wider spread.
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2

      // If the edge direction is reversed relative to the normalized one, flip the curvature
      // so every edge is laid out in the same reference frame and the directions don't overlap.
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on the number of edges between the pair.
      // Base distance is 150; add 50 per additional edge.
    ...
    // Attract toward the center so disconnected node clusters settle near the middle
  ...
  // Links - use <path> so we can draw curves
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc that starts at the node and returns to it
      const loopRadius = 30
      // Start to the right of the node and loop around
      const x1 = sx + 8  // Start offset
      const y1 = sy - 4
      const x2 = sx + 8  // End offset
      const y2 = sy + 4
      // Use an arc to draw the self-loop (sweep-flag=1 clockwise)
    ...
    // Compute curve control point - dynamically tuned by edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Offset perpendicular to the line, scaled to a fraction of the distance so the curve stays visible.
    // The more edges in the pair, the larger the proportion of distance we use.
  ...
  // Compute the midpoint of the curve (used to position labels)
  const getLinkMidpoint = (d) => {
    ...
    // Quadratic Bezier formula B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, with t=0.5
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // Reset the style of any previously selected edge
  ...
  // Link labels background (white background keeps the text legible)
  ...
  // Keep refs so external code can toggle visibility
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Just record the position; don't restart the simulation yet (distinguish click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect whether dragging has actually started (movement above threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart the simulation only on the first real drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only ease the simulation back to rest if there was actually a drag
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions (no rotation — horizontal is more legible)
