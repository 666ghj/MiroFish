<template>
  <div class="process-page">
    ...
    <div class="main-content">
      ...
      <!-- Right side: build pipeline details -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After documents are uploaded, the LLM analyzes their content and automatically generates an ontology (entity types + relation types) suitable for opinion simulation.
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Endpoint description</div>
                <div class="detail-content">
                  Based on the generated ontology, documents are chunked and the Zep API is called to build the knowledge graph and extract entities and relations.
                </div>
              </div>
...
// Current project ID (may transition from 'new' to the real ID)
...
// Initialize - either create a new project or load an existing one
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Continue polling an in-progress build
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Start polling graph data (independent of task-status polling)
      startGraphPolling()
...
// Start polling graph data
const startGraphPolling = () => {
  // Fetch immediately once
  fetchGraphData()

  // Then auto-fetch graph data every 10 seconds
...
const startPollingTask = (taskId) => {
  // Run an initial query immediately
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // Update the progress display to a completed state
        buildProgress.value = {
          progress: 100,
...
const renderGraph = () => {
  ...
  // Build a node lookup map by uuid
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  // Draw edges (with clickable transparent wide overlay)
  ...
  simulation.on('tick', () => {
    // Update positions of all edges (visible line and transparent click area)
