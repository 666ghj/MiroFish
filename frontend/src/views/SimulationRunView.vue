...
// Read maxRounds from the query parameter at init time so child components can receive it immediately
const maxRounds = ref(route.query.maxRounds ? parseInt(route.query.maxRounds) : null)
const minutesPerRound = ref(30) // Default of 30 minutes per round
...
const handleGoBack = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment is not running; check whether the process still needs to be stopped
...
const loadGraph = async (graphId) => {
  // While simulating, auto-refresh skips the full-screen loading indicator to avoid flicker.
  // Show the loading indicator only on manual refresh or initial load.
  if (!isSimulating.value) {
...
onMounted(() => {
  ...
  // Log the maxRounds config (the value was already read from the query param at init time)
  if (maxRounds.value) {
