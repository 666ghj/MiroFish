...
Converts entities from a Zep graph into the Agent Profile format required by the
OASIS simulation platform.

Optimizations:
1. Calls Zep retrieval to further enrich node information
2. Refines prompts to produce highly detailed personas
...
class OasisAgentProfile:
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        # Include the additional persona information when available
        if self.age:
    ...
    def to_twitter_format(self) -> Dict[str, Any]:
        ...
        # Include additional persona information
        if self.age:
            profile["age"] = self.age
        if self.gender:
            profile["gender"] = self.gender
...
class OasisProfileGenerator:
    ...
    2. Generates very detailed personas (basic information, career history, personality traits, social-media behavior, etc.)
    3. Distinguishes individual entities from abstract group entities
    """
    ...
    # Group/institution-type entities (require a representative-account persona)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use the Zep graph hybrid-search feature to obtain rich information related to an entity.

        Zep does not provide a built-in hybrid-search endpoint, so edges and nodes are
        searched separately and the results merged. The two searches run in parallel
        for efficiency.
        ...
        try:
            ...
            logger.info(f"Zep hybrid retrieval complete: {entity_name}, retrieved {len(results['facts'])} facts and {len(results['node_summaries'])} related nodes")
    ...
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine whether the entity is a group/institution type"""
    ...
    def _generate_profile_with_llm(
        ...
        - Individual entities: produce a concrete personal persona
        - Group/institution entities: produce a representative-account persona
        """
    ...
    def _fix_truncated_json(self, content: str) -> str:
        """Repair truncated JSON (output was cut off by max_tokens)"""
        import re
        ...
        # Simple heuristic: if the last character is not followed by a comma or closing bracket, the string may be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 1. First handle the truncation case
        content = self._fix_truncated_json(content)
        ...
        if json_match:
            ...
            # Find every string value and replace newlines within it
        ...
        # If we managed to extract meaningful content, mark it as repaired
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are an expert in generating social-media user profiles. Generate detailed, realistic personas for public-opinion simulation, staying as faithful as possible to the existing real-world facts. You must return valid JSON; no string value may contain unescaped newlines."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social-media user persona for the entity, staying as faithful as possible to the existing real-world facts.
...
Produce JSON containing the following fields:
...
2. persona: detailed persona description (2000 characters of plain text). Must include:
   ...
   - Personality traits (MBTI type, core personality, way of expressing emotion)
   - Social-media behavior (posting frequency, content preferences, interaction style, language quirks)
   - Stance and views (attitude toward the topic, content that may anger or move them)
   - Distinctive features (catchphrases, special experiences, personal hobbies)
   - Personal memory (an important part of the persona that describes the individual's connection to the event and their existing actions/reactions during it)
3. age: numeric age (must be an integer)
4. gender: gender; must be English: "male" or "female"
5. mbti: MBTI type (e.g., INTJ, ENFP)
6. country: country (use the localized name, e.g., "China")
7. profession: occupation
8. interested_topics: array of topics of interest

Important:
- Every field value must be a string or number; do not use newline characters
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social-media account persona for the institution/group entity, staying as faithful as possible to the existing real-world facts.
...
2. persona: detailed account-persona description (2000 characters of plain text). Must include:
   - Institutional basic information (formal name, organizational nature, founding background, primary functions)
   - Account positioning (account type, target audience, core function)
   - Communication style (language traits, common expressions, taboo topics)
   - Posting characteristics (content types, posting frequency, active time windows)
   ...
   - Institutional memory (an important part of the account persona that describes the institution's connection to the event and its existing actions/reactions during it)
3. age: fixed value of 30 (the institutional account's virtual age)
4. gender: fixed value "other" (institutional accounts use "other" to indicate non-personal)
5. mbti: MBTI type used to characterize the account style; e.g., ISTJ for rigorous and conservative
6. country: country (use the localized name, e.g., "China")
...
- persona must be a single coherent block of text and must not contain newline characters
...
- The institutional account's communication must match its identity"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID, used by Zep retrieval to obtain richer context
            parallel_count: Number of profiles to generate in parallel; defaults to 5
            realtime_output_path: File path for real-time writes (when provided, the file is rewritten after each profile is generated)
        ...
        # Helper that writes the profiles to disk in real time
        def save_profiles_realtime():
            """Save the profiles generated so far to the file"""
            if not realtime_output_path:
                return

            with lock:
                # Filter to the profiles that have been generated
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Persist the file in real time (even for fallback personas)
                    save_profiles_realtime()
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        ...
        # Only print to console (avoids duplication; logger no longer emits the full content)
        print(output)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter profiles in CSV format (matching the OASIS official requirements).

        OASIS Twitter required CSV fields:
        - user_id: user ID (zero-based, following CSV order)
        - name: real name
        - username: in-system username
        - user_char: detailed persona description (injected into the LLM system prompt to guide the agent's behavior)
        - description: short public bio (shown on the user's profile page)

        Difference between user_char and description:
        - user_char: internal use, the LLM system prompt that determines how the agent thinks and acts
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                # Strip newlines (replace with spaces in CSV)
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # Localized mappings (covers values that may come in from non-English LLM output)
        gender_map = {
            "男": "male",
            "女": "female",
            "机构": "other",
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        The user_id field is mandatory; OASIS agent_graph.get_agent() relies on it for matching.
    ...
    def save_profiles_to_json(
        ...
        """[Deprecated] Use the save_profiles() method instead"""
