...
Uses preset scripts plus LLM-based intelligent generation of configuration parameters.
"""
...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        Args:
            ...
            simulation_requirement: Description of the simulation requirement (used by the LLM to generate the configuration)
            document_text: Original document content (used by the LLM to understand the background)
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No qualifying entities were found; please check whether the graph was built correctly"
                self._save_simulation_state(state)
                return state
            ...
            profiles = generator.generate_profiles_from_entities(
                ...
                parallel_count=parallel_profile_count,  # Number of profiles to generate in parallel
            ...
            # Reddit was already saved in real time during generation; we save again here to ensure completeness
            if progress_callback:
                progress_callback(
            ...
            # ========== Stage 3: Intelligently generate simulation config via LLM ==========
            if progress_callback:
                progress_callback(
                    "generating_config", 0,
            ...
            # Note: the run scripts remain in the backend/scripts/ directory and are no longer copied into the simulation directory.
            # When starting the simulation, simulation_runner runs the scripts directly from scripts/.
