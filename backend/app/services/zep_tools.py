...
Wraps graph search, node read, edge query, and other tools used by the Report Agent.
...
2. PanoramaSearch (breadth search) — get the full panoramic view, including expired content
...
class InsightForgeResult:
    ...
    Contains the retrieval results for multiple sub-questions plus a synthesized analysis.
    """
    query: str
    ...
    def to_text(self) -> str:
        """Convert to a detailed text format the LLM can understand"""
        text_parts = [
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        if self.historical_facts:
            text_parts.append(f"\n### [Historical / Expired Facts] (record of how things evolved)")
...
class AgentInterview:
    """A single agent's interview result"""
    agent_name: str
    ...
    def to_text(self) -> str:
        ...
        # Show the full agent_bio, not truncated
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Truncate overly long content (cut at full-stop rather than hard truncate)
...
class InterviewResult:
    """
    Interview result
    Contains the interview answers from multiple simulation agents.
    """
    ...
    # Reasoning behind selecting these agents
    ...
    def to_text(self) -> str:
        """Convert to a detailed text format the LLM can understand and the report can cite"""
        text_parts = [
            "## In-Depth Interview Report",
            f"**Interview topic:** {self.interview_topic}",
            f"**Number of interviewees:** {self.interviewed_count} / {self.total_agents} simulation agents",
...
class ZepToolsService:
    ...
    4. interview_agents - in-depth interview (interview simulation agents to gather multi-perspective viewpoints)
    ...
    def _local_search(
        ...
        def match_score(text: str) -> int:
            """Compute the match score between text and the query"""
            if not text:
                return 0
            text_lower = text.lower()
            # Full match of the query
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Fetches all edges of the graph and filters those related to the given node.
        ...
        try:
            # Fetch all edges in the graph and filter
            all_edges = self.get_all_edges(graph_id)

            result = []
            for edge in all_edges:
                # Check whether the edge is related to the given node (as source or target)
    ...
    def get_entity_summary(
        ...
        """
        Get the relationship summary for the given entity

        Searches all information related to the entity and produces a summary.
    ...
    def get_simulation_context(
        ...
        """
        Get the simulation-related context

        Aggregates a search of all information relevant to the simulation requirement.

        Args:
            graph_id: Graph ID
            simulation_requirement: Simulation requirement description
            limit: Limit on the count of each information category
    ...
    def insight_forge(
        ...
        The most powerful hybrid retrieval; automatically decomposes the question
        ...
        3. Extract the relevant entities and fetch their details
        ...
        # Step 3: Extract relevant entity UUIDs from edges and fetch only those entities (not all nodes)
        entity_uuids = set()
        ...
        # Get details of all relevant entities (no count limit, full output)
        entity_insights = []
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are a professional question-analysis expert. Your task is to break a complex question into several sub-questions that can each be observed independently in the simulated world.
...
Return the sub-question list in JSON format."""
        ...
        try:
            ...
            # Fallback: variants based on the original question
            return [
                query,
                f"main participants of {query}",
                f"causes and impacts of {query}",
                f"how {query} unfolded"
            ][:max_queries]
    ...
    def panorama_search(
        ...
        Get the panoramic view, including all relevant content and historical/expired info:
        ...
        This tool is suited to scenarios that require a full view of an event and tracking of how it evolved.
        ...
        for edge in all_edges:
            ...
            if is_historical:
                # Historical / expired fact — annotate with time
    ...
    def interview_agents(
        ...
        Calls the real OASIS interview API to interview the agents currently running in the simulation:
        1. Automatically reads the persona files to learn about all simulation agents
        2. Uses the LLM to analyze the interview requirement and intelligently select the most relevant agents
        3. Uses the LLM to generate the interview questions
        4. Calls the /api/simulation/interview/batch endpoint for the real interview (interviewing on both platforms simultaneously)
        5. Aggregates all interview results and produces an interview report

        [Important] This feature requires the simulation to be running (the OASIS environment must not be shut down).

        [Use cases]
        - Need to understand opinions about an event from different role perspectives
        - Need to gather opinions and views from many sides
        - Need to obtain real answers from simulation agents (not LLM-generated)
        ...
        # Step 2: Use the LLM to choose which agents to interview (returns an agent_id list)
        ...
        INTERVIEW_PROMPT_PREFIX = (
            "You are being interviewed. Drawing on your persona, all of your past memory, and your actions, "
            ...
            "4. Answer each question in order, prefixing each answer with \"Question X:\" (where X is the question number)\n"
            "5. Separate answers to different questions with a blank line\n"
            "6. Each answer must contain substantive content — at least 2-3 sentences\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"

        # Step 4: Call the real interview API (no platform specified — defaults to interviewing both platforms)
        try:
            # Build the batch interview request list (no platform specified — both-platform interview)
            interviews_request = []
            ...
            # Call SimulationRunner's batch interview method (no platform — interview both platforms)
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Strip any tool-call JSON wrapping
                twitter_response = self._clean_tool_call_response(twitter_response)
                reddit_response = self._clean_tool_call_response(reddit_response)

                # Always render both-platform markers
                ...
                # Clean the response text: strip markers, numbering, Markdown, and other noise
                ...
                # Strategy 2 (supplementary): properly paired text inside Chinese quotation marks
                if not key_quotes:
                ...
                interview = AgentInterview(
                    ...
                    agent_bio=agent_bio[:1000],  # widen the bio length cap
            ...
            result.summary = f"Interview failed: {str(e)}. The simulation environment may have been shut down; please make sure the OASIS environment is running."
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Strip JSON tool-call wrapping from an agent reply and extract the actual content"""
        ...
        if os.path.exists(twitter_profile_path):
            try:
                with open(twitter_profile_path, 'r', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        # Convert CSV format into the unified format
                        profiles.append({
    ...
    def _select_agents_for_interview(
        ...
        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                - selected_agents: full info list for the chosen agents
                - selected_indices: index list of the chosen agents (used for the API call)
                - reasoning: rationale for the selection
        """
        ...
        system_prompt = """You are a professional interview-planning expert. Your task is to choose the most suitable interviewees from the simulation agent list based on the interview requirement.

Selection criteria:
1. The agent's identity/profession is relevant to the interview topic
2. The agent likely holds a unique or valuable point of view
3. Choose a diverse range of perspectives (e.g., supporters, opponents, neutrals, professionals)
4. Prefer roles directly involved in the event

Return JSON:
{
    "selected_indices": [list of indices of chosen agents],
...
Choose at most {max_agents} agents most suitable for the interview, and explain the reasoning."""
        ...
        try:
            ...
            # Get the full info for the selected agents
            selected_agents = []
            valid_indices = []
...
5. Each question must be no more than ~50 words — concise and clear
6. Ask the question directly; do not include background notes or prefixes
...
Roles of interviewees: {', '.join(agent_roles)}
...
Please generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        quote_instruction = "Use Chinese quotation marks 「」 when quoting an interviewee directly" if get_locale() == 'zh' else 'Use quotation marks "" when quoting interviewees'
        system_prompt = f"""You are a professional news editor. Based on the answers from multiple interviewees, produce an interview summary.

Summary requirements:
1. Distill the main points from each side
...
- {quote_instruction}
- You may use **bold** to emphasize keywords, but do not use any other Markdown syntax"""
...
Please produce the interview summary."""
