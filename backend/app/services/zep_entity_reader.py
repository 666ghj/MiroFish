...
Reads nodes from a Zep graph and filters out nodes matching predefined entity types
"""
...
class ZepEntityReader:
    ...
    3. Fetch each entity's related edges and connected-node info
    """
    ...
    def _call_with_retry(
        ...
        Args:
            ...
            max_retries: Maximum number of retries (default 3, meaning up to 3 attempts)
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Get all edges related to the given node (with retry)
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If a node's labels only contain "Entity", that entity does not match a predefined type — skip
        - If a node's labels contain any label other than "Entity" or "Node", it matches a predefined type — keep

        Args:
            graph_id: Graph ID
            defined_entity_types: Predefined entity-type list (optional; if provided, only these types are kept)
            enrich_with_edges: Whether to also fetch related-edge information for each entity
        ...
        # Fetch all edges (used for later relationship lookup)
        all_edges = self.get_all_edges(graph_id) if enrich_with_edges else []

        # Build node-UUID to node-data map
        node_map = {n["uuid"]: n for n in all_nodes}

        # Filter the matching entities
        filtered_entities = []
        ...
        for node in all_nodes:
            ...
            if enrich_with_edges:
                ...
                # Get basic info on related nodes
                related_nodes = []
