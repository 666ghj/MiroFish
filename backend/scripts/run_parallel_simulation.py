...
Runs Twitter and Reddit simulations concurrently from the same config file.

Features:
- Dual-platform (Twitter + Reddit) parallel simulation
- Does not immediately close the environment after the simulation completes; enters
...
# ============================================================
# Fix Windows encoding issues: set UTF-8 encoding before any imports.
...
if sys.platform == 'win32':
    ...
    # Reconfigure the standard streams to UTF-8 (fixes garbled CJK output in the console)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')

    # Force the default encoding (affects open()'s default encoding).
    # Note: this normally needs to be set at Python startup; setting it at runtime
    # may not take effect, so we also monkey-patch the built-in open function.
    import builtins
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        Wrap open() so that text-mode calls default to UTF-8 encoding.
        This works around third-party libraries (such as OASIS) that read files
        without specifying an encoding.
        """
        # Only set the default encoding for text mode (non-binary) when no encoding was specified
...
# Load the project root .env file (contains LLM_API_KEY and other config)
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai's max_tokens warnings (we intentionally do not set max_tokens, letting the model decide)"""
...
# Add the filter as soon as the module loads, so it is in effect before camel code runs
...
def disable_oasis_logging():
    ...
    OASIS logs are too noisy (recording each agent's observations and actions);
...
class ParallelIPCHandler:
    ...
    Manages both platforms' environments and processes Interview commands.
    """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: target platform (optional)
                ...
                - None / unspecified: interview on both platforms and return aggregated results
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Items that should be interviewed on both platforms
    ...
    async def process_commands(self) -> bool:
        ...
        Returns:
            True to continue running, False to indicate the loop should exit
        """
...
# Action type mapping (DB name -> standard name)
ACTION_TYPE_MAP = {
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    """
    Build an agent_id -> entity_name map from simulation_config.

    This makes actions.jsonl display real entity names rather than codenames like "Agent_0".
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: the largest rowid read previously (we use rowid rather than created_at,
            because the created_at format differs across platforms)
    ...
    Returns:
        ...
        - new_last_rowid: new largest rowid value
    """
    actions = []
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Slim down action_args, keeping only key fields (preserve full content; do not truncate)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    """
    Enrich an action with context info (post content, user names, etc).

    Args:
        cursor: database cursor
        action_type: action type
        action_args: action args (will be modified in place)
        agent_names: agent_id -> agent_name map
    """
    try:
        ...
        # Repost: enrich with the original post's content and author
        ...
        # Quote post: enrich with the original post's content, author and the quote comment
        ...
        # Failure to enrich context must not break the main flow
    ...
    try:
        ...
        if row:
            ...
            if agent_id is not None and agent_id in agent_names:
                ...
                # Read the name from the user table
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports a dual LLM configuration to speed up parallel simulations:
    ...
    When the boost LLM is configured, parallel simulations can route different platforms
    to different API providers, increasing concurrent throughput.

    Args:
        config: simulation config dict
        use_boost: whether to use the boost LLM config (if available)
    """
    # Check whether a boost config exists
    boost_api_key = os.environ.get("LLM_BOOST_API_KEY", "")
    boost_base_url = os.environ.get("LLM_BOOST_BASE_URL", "")
    boost_model = os.environ.get("LLM_BOOST_MODEL_NAME", "")
    has_boost_config = bool(boost_api_key)

    # Choose which LLM to use based on the flag and the available configs
    ...
    # If .env did not provide a model name, fall back to the config
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: maximum simulation rounds (optional, used to truncate overly long runs)
    ...
    # Build agent_id -> real name map from the config (uses entity_name instead of the default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # If the config doesn't include a given agent, fall back to OASIS's default name
    ...
    # Record start of round 0 (initial-events phase)
    if action_logger:
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # When there are no active agents, still record round end (actions_count=0)
            if action_logger:
        ...
        # Read the actions actually executed from the database and record them
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM config (if available; otherwise falls back to the standard config)
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum simulation rounds (optional, used to truncate overly long runs)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
    ...
    # Create the shutdown event at the start of main so the entire program can respond to shutdown signals
    ...
    # Hold both platforms' simulation results
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                try:
                    ...
                    pass  # timed out, continue the loop
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers so SIGTERM/SIGINT cause an orderly shutdown.

    Persistent simulation scenario: after the simulation completes the process does not
    exit, but waits for interview commands. When a termination signal arrives, we need to:
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set the event to notify the asyncio loop to exit (so it can clean up resources)
            if _shutdown_event:
                _shutdown_event.set()

        # Do not call sys.exit() directly; let the asyncio loop exit normally and clean up resources.
        # Only force-exit if the signal repeats.
        else:
            print("Forcing exit...")
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean up the multiprocessing resource tracker (avoids warnings on exit)
        try:
