...
This script reads parameters from a config file and runs a fully-automated simulation.
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        Always uses configuration from the project root .env file (highest priority):
        - LLM_API_KEY: API key
        ...
        # If not present in .env, fall back to the config
        if not llm_model:
    ...
    def _get_active_agents_for_round(
        ...
        # Adjust based on time-of-day band
        ...
        # Compute activation probability based on each Agent's config
        candidates = []
        for cfg in agent_configs:
            ...
            # Check whether currently within active hours
            if current_hour not in active_hours:
                continue

            # Roll probability based on activity level
    ...
    async def run(self, max_rounds: int = None):
        ...
        Args:
            max_rounds: maximum number of simulation rounds (optional, used to truncate overly long runs)
        """
        print("=" * 60)
        ...
        for round_num in range(total_rounds):
            ...
            # Get the Agents activated this round
...
def setup_signal_handlers():
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            ...
            # Force-exit only if signal repeats
            print("Forcing exit...")
            sys.exit(1)
